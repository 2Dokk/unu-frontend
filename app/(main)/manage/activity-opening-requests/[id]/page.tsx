...
export default function ActivityOpeningRequestManagementDetailPage() {
  ...
  const [request, setRequest] =
    useState<ActivityOpeningRequestResponse | null>(null);
  ...
  async function review(
    ...
    try {
      ...
      const updated = await reviewActivityOpeningRequest(
        id,
        status,
        comment.trim(),
      );
  ...
  async function approve() {
    ...
    const isStudy = request.activityType.code === "STUDY";
    const isSpecialLecture =
      request.activityType.code === "SPECIAL_LECTURE";

    const usesDeposit = isStudy || isSpecialLecture;

    if (usesDeposit && depositAmount.trim() === "") {
      toast.error("참여 보증금을 설정해주세요.");
      return;
    }
    ...
    if (
      usesDeposit &&
      (!Number.isInteger(parsedDepositAmount) ||
        parsedDepositAmount < 0)
    ) {
      toast.error("참여 보증금은 0원 이상으로 입력해주세요.");
      return;
    }
    ...
    try {
      ...
      const updated = await approveActivityOpeningRequest(
        id,
        {
  ...
  const isProject =
    request.activityType.code === "PROJECT";
        ...
        <div>
          <div className="mb-2 flex items-center gap-2">
            ...
            {request.personalProject && (
              <span className="text-xs font-medium text-[#264638]">
                개인 프로젝트
              </span>
            )}
          </div>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="보완할 내용이나 승인 의견을 작성하세요."
                />
              </div>
